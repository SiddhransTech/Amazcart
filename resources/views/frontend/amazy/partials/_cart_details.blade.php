                                    ...
                                    @foreach($items as $cart)
                                        @if($cart->product_type == 'product' && $cart->product)
                                            @if($cart->is_select == 1)
                                                @php
                                                    $pro_price = 0;
                                                    if (isModuleActive('WholeSale') && $cart->product->wholeSalePrices && $cart->product->wholeSalePrices->count()) {
                                                        $w_main_price = 0;
                                                        foreach ($cart->product->wholeSalePrices as $w_p) {
                                                ...
                                                <div class="row gutters-5 align-items-center m-0">
                                                    <div class="col-lg-5 d-flex p-0">
                                                        <a href="{{ singleProductURL($cart->seller->slug ?? '', $cart->product->product->slug ?? '') }}" class="d-flex align-items-center gap_20 cart_thumb_div">
                                                            <div class="thumb">
                                                                <img src="{{ showImage($cart->product->sku->variant_image ?? $cart->product->product->thumbnail_image_source) }}"
                                                                     alt="{{ textLimit($cart->product->product->product_name, 30) }}"
                                                                     title="{{ textLimit($cart->product->product->product_name, 30) }}">
                                                            </div>
                                                            <div class="summery_pro_content">
                                                                ...
                                                                <p class="font_14 f_w_400 m-0">
                                                                    @if($cart->product->product->product_type == 2)
                                                                ...
                                                                <div class="d-flex flex-wrap align-items-center gap-10 mt-3">
                                                                    <div class="d-block d-lg-none">
                                                                        <div class="product_number_count style_4" data-target="amount-3">
                                                                            <button class="count_single_item inumber_decrement change_qty"
                                                                            ...
                                                                            <input name="qty[]" id="qty_{{ $cart->id }}" maxlength="12"
                                                                                   data-value="{{ $cart->qty }}"
                                                                            ...
                                                                            <input type="hidden" id="maximum_qty_{{ $cart->id }}"
                                                                                   value="{{ $cart->product->product->max_order_qty }}">
                                                                            <input type="hidden" id="minimum_qty_{{ $cart->id }}"
                                                                                   value="{{ $cart->product->product->minimum_order_qty }}">
                                                                            <button class="count_single_item number_increment change_qty"
                                                                                    data-qty_id="#qty_{{ $cart->id }}"
                                                                                    data-change_amount="1"
                                                                                    data-maximum_qty="#maximum_qty_{{ $cart->id }}"
                                                                                    data-minimum_qty="#minimum_qty_{{ $cart->id }}"
                                                                            ...
                                                                            @if(isModuleActive('WholeSale'))
                                                                                <input type="hidden" id="getWholesalePrice_{{ $cart->id }}"
                                                    ...
                                                    <div class="col-lg col-4 order-1 order-lg-0 my-3 my-lg-0 d-none d-lg-block">
                                                        ...
                                                        @if($cart->product->product->hasDeal && $cart->product->product->hasDeal->discount > 0)
                                                        ...
                                                        @elseif($cart->product->product->hasDiscount == 'yes')
                                                            <span class="green_badge text-nowrap">
                                                        ...
                                                        @endif
                                                        <h4 class="font_16 f_w_700 m-0 text-nowrap set_base_price{{ $cart->id }}">{{ single_price($pro_price ?? $cart->product->sell_price) }}</h4>
                                                        <input type="hidden" class="get_base_price{{ $cart->id }}"
                                                               value="{{ single_price($pro_price ?? $cart->product->sell_price) }}">
                                                    </div>
                                                    ...
                                                    <div class="col-lg-auto order-5 order-lg-0 text-end">
                                                        <span class="close_icon style_2 lh-1 cart_item_delete_btn cursor_pointer"
                                                              data-id="{{ $cart->id }}"
                                        ...
                                        @elseif($cart->product_type == 'gift_card' && $cart->giftCard)
                                            @if($cart->is_select == 1)
                                                @php
                                                    $gift_price = is_null($cart->giftCard->type) ? $cart->giftCard->sell_price : $cart->giftCard->addGiftCardInfo->gift_selling_price;
                                                    $subtotal += $gift_price * $cart->qty;
                                                @endphp
                                            @endif
                                            <li class="list-group-item px-0 px-lg-3 mb_10">
                                                <div class="row gutters-5 align-items-center">
                                                    <div class="col-lg-5 d-flex">
                                                        <a href="{{ is_null($cart->giftCard->type) ? route('frontend.gift-card.show', $cart->giftCard->sku) : route('frontend.gift-card.show.multiple', $cart->giftCard->sku) }}"
                                                           class="d-flex align-items-center gap_20 cart_thumb_div">
                                                            <div class="thumb">
                                                                <img src="{{ showImage($cart->giftCard->thumbnail_image) }}"
                                                                     alt="{{ textLimit($cart->giftCard->name, 30) }}"
                                                                     title="{{ textLimit($cart->giftCard->name, 30) }}">
                                                            </div>
                                                            <div class="summery_pro_content">
                                                    ...
                                                    <div class="col-lg col-6 order-4 order-lg-0">
                                                        <div class="product_number_count style_4" data-target="amount-3">
                                                            ...
                                                            <input name="qty[]" id="qty_{{ $cart->id }}"
                                                                   data-value="{{ $cart->qty }}"
                                                                   value="{{ getNumberTranslate($cart->qty) }}"
                                                                   class="count_single_item input-number qty"
                                                                   type="text"
                                        ...
                                        @elseif($cart->is_select == 1)
                                            @php
                                                \Log::warning('Invalid cart item type or missing relationships', [
                                                    'cart_id' => $cart->id,
                                                    'product_type' => $cart->product_type,
                                                    'product_id' => $cart->product_id,
                                                    'has_product' => !empty($cart->product),
                                                    'has_giftCard' => !empty($cart->giftCard)
                                                ]);
                                                continue;
                                            @endphp
                                        @endif
                                                ...
                                                <li class="list-group-item px-0 px-lg-3 mb_10">
                                                    <div class="row gutters-5 align-items-center m-0">
                                                        ...
                                                        <div class="col-lg col-4 order-1 order-lg-0 my-3 my-lg-0 d-none d-lg-block">
                                                            ...
                                                            @endif
                                                            <h4 class="font_16 f_w_700 m-0 set_base_price{{ $cart->id }}">{{ single_price($pro_price ?? $cart->product->sell_price) }}</h4>
                                                            <input type="hidden" class="get_base_price{{ $cart->id }}"
                                                                   value="{{ single_price($pro_price ?? $cart->product->sell_price) }}">
                                                        </div>
                                                        <div class="col-lg col-6 order-4 order-lg-0 d-none d-lg-block">
            ...
            <div class="subtotal_lists">
                <div class="single_total_list d-flex align-items-center">
                    ...
                    <div class="single_total_right">
                        <span>+ {{ single_price(app('general_setting')->price_with_vat ? $subtotal - $tax : $subtotal) }}</span>
                    </div>
                </div>
                ...
                <div class="single_total_list d-flex align-items-center flex-wrap">
                    ...
                    <div class="single_total_right">
                        <span>{{ app('general_setting')->price_with_vat ? '+ '.single_price($tax) : __('defaultTheme.calculated_at_next_step') }}</span>
                    </div>
                </div>
                <div class="total_amount d-flex align-items-center flex-wrap">
                    ...
                    <div class="single_total_right">
                        <span class="total_text" id="grand_total" data-amount="{{ str_replace(',', '', number_format($grand_total, 2)) }}">
